...
# Fallback for environments without CUDA (e.g., macOS CPU/MPS).
...
class Coach:
	...
	def prepareModel(self):
		...
		cond_dim = args.cond_dim if args.conditional_diffusion else 0
		
		out_dims = eval(args.dims) + [args.item]
		in_dims = out_dims[::-1]
		self.denoise_model_image = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm, cond_dim=cond_dim, cond_dropout=args.cond_dropout).cuda()
		self.denoise_opt_image = torch.optim.Adam(self.denoise_model_image.parameters(), lr=args.lr, weight_decay=0)

		out_dims = eval(args.dims) + [args.item]
		in_dims = out_dims[::-1]
		self.denoise_model_text = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm, cond_dim=cond_dim, cond_dropout=args.cond_dropout).cuda()
		self.denoise_opt_text = torch.optim.Adam(self.denoise_model_text.parameters(), lr=args.lr, weight_decay=0)

		if args.data == 'tiktok':
			out_dims = eval(args.dims) + [args.item]
			in_dims = out_dims[::-1]
			self.denoise_model_audio = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm, cond_dim=cond_dim, cond_dropout=args.cond_dropout).cuda()
			self.denoise_opt_audio = torch.optim.Adam(self.denoise_model_audio.parameters(), lr=args.lr, weight_decay=0)

	def build_condition(self, batch_item, modal_feats):
		if not args.conditional_diffusion:
			return None
		denom = batch_item.sum(dim=1, keepdim=True) + 1e-8
		return torch.mm(batch_item, modal_feats) / denom
	...
	def trainEpoch(self):
		...
		for i, batch in enumerate(diffusionLoader):
			...
			cond_image = self.build_condition(batch_item, image_feats)
			cond_text = self.build_condition(batch_item, text_feats)
			if args.data == 'tiktok':
				cond_audio = self.build_condition(batch_item, audio_feats)
			...
			diff_loss_image, gc_loss_image = self.diffusion_model.training_losses(self.denoise_model_image, batch_item, iEmbeds, batch_index, image_feats, cond=cond_image)
			diff_loss_text, gc_loss_text = self.diffusion_model.training_losses(self.denoise_model_text, batch_item, iEmbeds, batch_index, text_feats, cond=cond_text)
			if args.data == 'tiktok':
				diff_loss_audio, gc_loss_audio = self.diffusion_model.training_losses(self.denoise_model_audio, batch_item, iEmbeds, batch_index, audio_feats, cond=cond_audio)
		...
		with torch.no_grad():
			...
			for _, batch in enumerate(diffusionLoader):
				...
				# image
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_image, batch_item, args.sampling_steps, args.sampling_noise, cond=cond_image)
				...
				# text
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_text, batch_item, args.sampling_steps, args.sampling_noise, cond=cond_text)
				top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)
				...
				if args.data == 'tiktok':
					...
					cond_audio = self.build_condition(batch_item, audio_feats)
					# audio
					denoised_batch = self.diffusion_model.p_sample(self.denoise_model_audio, batch_item, args.sampling_steps, args.sampling_noise, cond=cond_audio)
					top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)
