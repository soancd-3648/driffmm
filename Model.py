...
class Denoise(nn.Module):
	def __init__(self, in_dims, out_dims, emb_size, norm=False, dropout=0.5, cond_dim=0, cond_dropout=0.0, cond_norm=True):
		...
		if self.cond_dim > 0:
			self.cond_layer = nn.Linear(self.cond_dim, self.time_emb_dim)
			self.cond_drop = nn.Dropout(cond_dropout)
		else:
			self.cond_layer = None

		input_dim = self.in_dims[0] + self.time_emb_dim
		if self.cond_dim > 0:
			input_dim += self.time_emb_dim
		in_dims_temp = [input_dim] + self.in_dims[1:]
	...
	def init_weights(self):
		...
		if self.cond_layer is not None:
			size = self.cond_layer.weight.size()
			std = np.sqrt(2.0 / (size[0] + size[1]))
			self.cond_layer.weight.data.normal_(0.0, std)
			self.cond_layer.bias.data.normal_(0.0, 0.001)
	...
	def forward(self, x, timesteps, mess_dropout=True, cond=None):
		...
		if self.cond_layer is not None and cond is not None:
			if self.cond_norm:
				cond = F.normalize(cond)
			cond = self.cond_drop(cond)
			cond_emb = self.cond_layer(cond)
			h = torch.cat([x, emb, cond_emb], dim=-1)
		else:
...
class GaussianDiffusion(nn.Module):
	...
	def p_mean_variance(self, model, x, t, cond=None):
		model_output = model(x, t, False, cond=cond)
	...
	def training_losses(self, model, x_start, itmEmbeds, batch_index, model_feats, cond=None):
		...
		model_output = model(x_t, ts, cond=cond)
